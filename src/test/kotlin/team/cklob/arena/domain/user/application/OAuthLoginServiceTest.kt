...
class OAuthLoginServiceTest : DescribeSpec({
    ...
    it("기존 OAuth 사용자는 access와 refresh token을 받는다") {
        ...
        val user =
            User(
                ...
                oauthProviderUserId = "provider-id",
            ).apply {
                id = 1L
            }
    ...
    it("Apple mock provider의 Android authorization code와 state를 교환한다") {
        ...
        val service =
            OAuthLoginServiceImpl(
                OAuthProviderClientResolver(listOf(client)),
                users,
                mockk<RefreshSessionRepository>(),
                tokens,
                properties,
