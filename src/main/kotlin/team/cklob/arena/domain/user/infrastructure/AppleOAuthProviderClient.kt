...
class AppleOAuthProviderClient(
    ...
    override fun authenticate(
        ...
        try {
            ...
            val verifier = if (platform == ClientPlatform.ANDROID) state?.let(stateStore::consume) ?: throw ExpectedException(SecurityErrorCode.INVALID_TOKEN) else null
            if (platform == ClientPlatform.ANDROID && redirectUri == null) throw ExpectedException(SecurityErrorCode.INVALID_TOKEN)
            val token =
                restClientBuilder
                    .build()
                    .post()
    ...
    private fun createClientSecret(apple: AppleOAuthProperties, clientId: String): String {
        val now = Instant.now()
        return Jwts
            .builder()
            .issuer(apple.teamId)
    ...
    private fun verifyIdToken(idToken: String, clientId: String) =
        Jwts
            .parser()
