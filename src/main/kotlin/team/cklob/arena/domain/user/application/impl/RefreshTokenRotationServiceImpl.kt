...
class RefreshTokenRotationServiceImpl(
    ...
    override fun execute(refreshToken: String): TokenPair {
        ...
        if (session.user.id != userId || session.tokenHash != tokenHash || (cachedTokenHash != null && cachedTokenHash != tokenHash) || session.revokedAt != null || !session.expiresAt.isAfter(now)) {
        ...
        val newSession = refreshSessionRepository.save(RefreshSession(session.user, "pending", now.plus(jwtProperties.refreshTokenExpiration)))
