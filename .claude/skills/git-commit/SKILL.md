...
allowed-tools: Bash(git status:*), Bash(git diff:*), Bash(git add:*), Bash(git commit:*), Bash(git branch:*), Bash(git rev-parse:*), Bash(git log:*), Bash(git config:*), Read
---
...
If the current branch is `main`, `master`, or `develop`, warn the user before committing directly and ask for confirmation (`AskUserQuestion`). Prefer committing on a feature/fix branch.
...
**어트리뷰션 주의**: 커밋 메시지에 "Co-Authored-By", "Generated with Claude Code" 같은 트레일러나 서명을 추가하지 않는다. 커밋 작성자는 로컬 `git config user.name`/`user.email`(사용자 본인 계정)을 그대로 따른다 — 별도로 identity를 바꾸지 않는다.
