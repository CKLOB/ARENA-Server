...
| 스킬                                            | 설명                                                              |
|-----------------------------------------------|-----------------------------------------------------------------|
| [git-commit](./git-commit/SKILL.md)           | 변경사항을 논리적 단위로 분리해 컨벤션(`<type> :: <한글 요약>`)에 맞는 커밋 생성. Git Flow 자동 감지(develop/main 직접 커밋 경고) |
| [write-pr](./write-pr/SKILL.md)               | base 브랜치(develop) 이후 커밋 기반으로 PR 제목(접두어 없는 한글)·본문·라벨 생성 및 GitHub PR 오픈까지 자동화 |
